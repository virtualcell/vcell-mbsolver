...
`python/pyvcellmbsolver.cpp` is the low-level pybind11 module (`vcellmbsolver_py`) exposing C++ types directly. `python/vcellmbsolver.py` is the high-level wrapper: `MovingBoundarySolver` facade plus `TimeStepObserver` / `SimulationObserver` abstract bases whose subclasses adapt to the C++ `*Client` interfaces. Python tests are in `python/tests/`.

### Python packaging (wheels)

`pyproject.toml` uses the `scikit-build-core` backend to drive the existing CMake build into a wheel (`pip install .` / `python -m build`). It builds only the `vcellmbsolver_py` target and installs only the `python` install component (extension + wrapper) — the C++ lib/binary/headers are excluded; tests and messaging are forced off. `python/CMakeLists.txt` switches the install destination to `.` when `SKBUILD` is defined (wheel build) vs. `Python3_SITEARCH` for a plain `cmake --install`. The wheel version is scraped from `project(VCellMovingBoundary VERSION ...)` in the root `CMakeLists.txt`, so that line is the single source of truth. `.github/workflows/wheels.yml` runs cibuildwheel across Linux/macOS/Windows; a `publish` job does PyPI trusted-publishing on `v*` tags only.
